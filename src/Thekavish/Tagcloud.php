...
class Tagcloud extends Facade
{
     ...
     * @return bool|mixed
     */
    public function addTag($tagAttributes = [])
    {
    ...
    {
        $this->_attributes = array_unique(array_merge($this->_attributes, array_keys($attributes)));
    }
    ...
    public function render($returnType = 'html')
    {
        ...
        if (count($this->_tagsArray)) {
            ...
            foreach ($this->_tagsArray as $tag => $arrayInfo) {
                ...
                $sizeClass          = $this->_getClassFromPercentAlt(($arrayInfo['size'] / $max) * 100);
                $arrayInfo['range'] = $sizeRange;
                $arrayInfo['class'] = $sizeClass;
                if ($returnType == 'array') {
                    $return [$tag] = $arrayInfo;
                } elseif ($returnType == 'html') {
                    $return .= $this->htmlizeTag($arrayInfo, $sizeRange, $sizeClass);
                }
            }
    ...
    /**
     * Reduces the array by removing strings with a length shorter than the minLength
     *
     * @returns array $this->_tagsArray
     */
     ...
     *
     * @returns int $class The respective class name based on the percentage value
     */
    ...
    /**
     * Get the class group range using a percentage.
     *
     * @param  int  $percent
     *
     * @return string $class The respective class name based on the percentage value
     */
    protected function _getClassFromPercentAlt($percent)
    {
        $percent = floor($percent);
        if ($percent < 20) {
            $class = 'smallest';
     ...
     * This function is mainly an anchor to decide if a user-supplied custom function should be used or the normal output method.
     *
     ...
     * @return string
     */
    public function htmlizeTag($arrayInfo, $sizeRange, $sizeClass)
    {
        if (isset($this->_htmlizeTagFunction)) {
            ...
            return $htmlizer($arrayInfo, $sizeRange, $sizeClass);
        } else {
            return "<span class='{$sizeClass} tag size{$sizeRange}'> &nbsp; {$arrayInfo['tag']} &nbsp; </span>";
        }
    }

    /**
     * Calculate the class given to a tag from the weight percentage of the given tag.
     *
     * @param $percent
     *
     * @return int
     ...
     * @return string
     */
    public function calculateClassFromPercentAlt($percent)
    {
        return $this->_getClassFromPercentAlt($percent);
    }
